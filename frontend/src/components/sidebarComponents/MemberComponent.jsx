  ...
  return (
    <div className='w-full h-full bg-[var(--bg-color)] flex flex-col items-center pt-[3rem] sm:pt-[40px] pb-[20vw] sm:pb-[100px] '>
        ...
        <div className='relative w-full grid grid-cols-2 md:grid-cols-3  mt-[1rem] sm:mt-9 gap-[0.1rem] px-[0.5rem] pb-[6rem] sm:pb-[120px]'>
            ...
            {!userLoading && !userError && users.length > 0 ? (users.map((user, index)=>{
                ...
                return <div key={index} className='border-1 mt-[0.8rem]  sm:mt-[12px] mx-auto w-[96%] pt-[1.5rem] sm:pt-[30px] sm:pb-[22px] pb-[1.2rem] border-gray-400 text-center flex flex-col justify-between items-center rounded-lg'>
                        <div className=''>
                            <img src={user.profilePic} alt='Profile Pic' className='rounded-full w-[5.7rem] mx-auto sm:w-28 '/>
                            <h4 className='text-[1.1rem] sm:text-[1.2rem] font-semibold mt-2 capitalize'>{user.firstName} {user.lastName}</h4>
                            <p className='whitespace-pre-wrap w-[90%] sm:w-[90%] text-[0.88rem] sm:text-[0.97rem] text-[var(--dark-shade)] mx-auto'>{user.about}</p>
                        </div>
                        {IsRequested ? <button onClick={() => pendingBtnHandler(user._id)} className='border-2 border-gray-600 text-[var(--text-color)] flex gap-1 justify-center items-center w-[80%] mt-[0.9rem] cursor-pointer rounded-full p-[0.5rem] text-[0.85rem] sm:p-[0.6rem] max-w-[220px] sm:mt-3 sm:text-[16px] font-semibold'><i className="ri-time-line text-[0.9rem] sm:text-[1rem] leading-6 "></i>Pending</button> 
